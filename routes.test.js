...
describe("PATCH /users/:username", () => {
  test("allows a user to update their own information", async () => {
    const response = await request(app)
      .patch("/users/u1") // Assuming 'u1' is the username of the regular user
      .send({
...
//Bug3
describe("PATCH /users/:username", () => {
  test("rejects updates with non-allowed fields", async () => {
    const response = await request(app)
      .patch("/users/u1") // Assuming 'u1' is a valid username
      .send({
        first_name: "NewFirstName",
        notAllowedField: "ThisShouldNotBeHere",
        "_token": tokens.u1
      })
...
//Bug4
test('successfully deletes a user when requested by an admin', async () => {
  await request(app)
      .post("/auth/register")
      .send({
        username: "testuser",
  ...
  const response = await request(app)
    ...
    .delete('/users/testuser') // Adjust the endpoint as necessary
    .send({ _token: tokens.u3 }); // Include the token in the request body
...
test('returns a 404 error if the user does not exist', async () => {
  const response = await request(app)
    .delete('/users/nonexistentuser') // Attempting to delete a user that does not exist
    .send({ _token: tokens.u3 }); // Include the token in the request body
